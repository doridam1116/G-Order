...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    private void handleAuthenticationError(HttpServletResponse response) throws IOException {
        ...
        response.getWriter().write(PropertyConfig.getMessageSource().getMessage("HTTP_UNAUTHORIZED",null, Locale.getDefault()));
    }
    ...
    private void handleAuthorizationError(HttpServletResponse response) throws IOException {
        ...
        response.setContentType(MediaType.APPLICATION_JSON_VALUE);
        response.setCharacterEncoding("UTF-8");
        response.getWriter().write(PropertyConfig.getMessageSource().getMessage("HTTP_FORBIDDEN",null, Locale.getDefault()));
    }
